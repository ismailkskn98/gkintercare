...
export default function HeroSection({ content }) {
  ...
  return (
    <section className="gridContainer relative bg-primary pt-20 text-white">
          ...
          <div className="grid gap-3">
            <label className="grid gap-2 text-xs font-800 uppercase tracking-[0.1em] text-primary/62">
              ...
              <span className="flex items-center gap-3 rounded-[8px] border border-primary/10 bg-light-bg px-4 py-3">
                ...
                <input
                  className="w-full bg-transparent text-sm font-600 text-primary outline-none placeholder:text-muted"
      ...
      <div className="relative z-20 -mb-14 grid min-h-32 gap-px overflow-hidden rounded-[8px] border border-white/72 bg-primary/10 shadow-[0_24px_70px_rgba(11,60,93,0.18)] md:grid-cols-4">
        {hero.trustItems.map((item, index) => {
          const Icon = trustIcons[index] || ShieldCheck;

          return (
            <article className="flex items-center gap-4 bg-white px-6 py-7" key={item.title}>
