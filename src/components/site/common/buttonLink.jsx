...
const variants = {
  primary:
    "bg-accent text-white! shadow-[0_16px_34px_rgba(200,169,105,0.32)] hover:bg-[#b99855]",
...
const motionReduce =
  "motion-reduce:transition-none motion-reduce:group-hover/action-link:translate-x-0 motion-reduce:group-hover/action-link:translate-y-0";
...
export function AnimatedLabel({ children }) {
  return (
    <span className="relative inline-flex h-5 max-w-full overflow-hidden">
      <span
        className={cx(
          "flex flex-col will-change-transform transition-transform group-hover/action-link:-translate-y-5",
          motionDuration,
          motionEase,
          motionReduce,
        )}
...
export function AnimatedIcon({ icon: Icon, className }) {
  ...
  return (
    <span className="relative inline-flex size-4 shrink-0 overflow-hidden">
      <span
        className={cx(
          "flex w-8 -translate-x-4 will-change-transform transition-transform group-hover/action-link:translate-x-0",
          motionDuration,
          motionEase,
          motionReduce,
        )}
...
export default function ButtonLink({
  ...
  const classes = cx(
    "focus-ring group/action-link inline-flex min-h-11 items-center justify-center gap-2 rounded-[8px] px-5 py-3 text-sm font-800 transition-colors duration-300 ease-out",
    variants[variant],
    className,
  );
