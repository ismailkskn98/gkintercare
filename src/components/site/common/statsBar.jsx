...
export default function StatsBar({ stats, className = "", decorated = true, decoratedClassName = "", decoratedColor = "%230b3c5d" }) {
  const sectionRef = useRef(null);
  const counterInView = useInView(sectionRef, { once: true, amount: 0.3 });

  return (
    <section className={`relative gridContainer bg-white ${className} py-[clamp(2.75rem,5vw,4rem)]`} ref={sectionRef}>
      {decorated ? (
        <>
          <article className={cn("bg-primary absolute top-0 left-0 w-16 h-16 fluid rounded-br-3xl", decoratedClassName)}>
            <Image
              src={`data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' width='24' height='24' fill='none' viewBox='0 0 24 24'%3E%3Cpath fill='${decoratedColor}' d='M0 24h24C10.745 24 0 13.255 0 0z'/%3E%3C/svg%3E`}
              width={24}
              height={24}
              alt="border cornea"
          ...
          <article className={cn("bg-primary absolute top-0 right-0 w-16 h-16 fluid rounded-bl-3xl", decoratedClassName)}>
            <Image
              src={`data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' width='24' height='24' fill='none' viewBox='0 0 24 24'%3E%3Cpath fill='${decoratedColor}' d='M0 24h24C10.745 24 0 13.255 0 0z'/%3E%3C/svg%3E`}
              width={24}
              height={24}
          ...
          return (
            <article className="flex flex-col items-center lg:items-start text-center lg:text-left gap-4" key={`${value}-${label}`}>
              ...
              <div className="shrink-0 text-center lg:text-left lg:w-36">
                {isMetric ? (
                  <StatCounter value={stat.value} inView={counterInView} delay={index * 120} variant="bar" />
                ) : (
                  <strong className="block text-xl font-800 leading-none tracking-tight text-primary xl:text-2xl">{value}</strong>
                )}
