import { MotionSlideUp } from "./animation";
...
export default function CompactPageIntro({ label, title, accent, description, trustStrip = [] }) {
  return (
    <section className="gridContainer relative overflow-hidden bg-white pt-[clamp(7.75rem,10vw,7.5rem)] text-foreground">
      ...
      <MotionSlideUp className="relative z-10 pb-[clamp(1.25rem,3vw,2rem)] pt-[clamp(1rem,2.5vw,1.75rem)]">
        <div className="mx-auto flex max-w-5xl flex-col items-center text-center">
          <span className="section-label">{label}</span>
          <h1 className="mt-4 max-w-5xl text-[clamp(2.25rem,6vw,4.05rem)] font-800 leading-[1.03] [text-wrap:balance]">
            {title}
            {accent ? <span className="serif-accent mt-1 block text-accent">{accent}</span> : null}
          </h1>
          {description ? <p className="mx-auto mt-5 max-w-3xl text-[clamp(0.95rem,1.35vw,1.05rem)] leading-7 text-muted md:leading-8">{description}</p> : null}
          <div className="mt-7 h-px w-full max-w-80 bg-linear-to-r from-transparent via-primary/14 to-transparent" />
        </div>
      </MotionSlideUp>

      {trustStrip.length > 0 ? (
        <MotionSlideUp className="fluid" delay={0.06}>
